...
CONFIG      = "configs/your_6d_config.json"
BASE        = "models/your_6d_run_prefix"
...
M_cap    = np.array(eq["M_cap"],    dtype=np.float64)
phi_star = np.array(eq["phi_star"], dtype=np.float64)
phi_mask = np.array(eq.get("phi_mask", (M_cap > 0).astype(float)), dtype=np.float64)
beta_cfg = np.array(eq["beta"],     dtype=np.float64)
Sigma    = np.array(eq["Sigma"],    dtype=np.float64)
w        = np.array(eq["w"],        dtype=np.float64)
c        = np.array(eq["c"],        dtype=np.float64)
u_lower  = np.array(eq["u_lower"],  dtype=np.float64)
u_upper  = np.array(eq["u_upper"],  dtype=np.float64)
gamma    = float(eq["discount"])
x0_val   = float(eq.get("x0", 1.0))

u_star   = np.array(eq.get("u_star", np.ones(K)), dtype=np.float64)

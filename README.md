...
controlled $2\times2$ Monte Carlo cross-evaluation using common random

numbers;



the deployment comparison given by the diagonal of that table; and



a critic-free comparison under $\hat u\equiv0$.
...
The 2D, 6D, and 30D evaluations use 40,000 Monte Carlo trajectories per
policy. Because of the substantially higher computational cost, the 100D
evaluation uses 8,000 trajectories per policy. All policy comparisons within
the same configuration use common random numbers, so the reported differences
remain paired comparisons even though the 100D sample size is smaller.
...
RBMSolver, developed by Nian Si for
the numerical study of drift control in high-dimensional reflected Brownian
motion. Permission was granted by the RBMSolver author to release this
modified implementation under a license chosen by the present author.

RBMSolver was itself adapted from
DeepPDE_ActorCritic,
which is distributed under the MIT License.

Relative to RBMSolver, this repository adds the boundary-jump adjoint
correction, routing-policy optimization, multiclass and multi-pool queueing
configurations, high-dimensional experiments, and the controlled and
critic-free Monte Carlo evaluation routines. Any errors introduced in these
modifications are the responsibility of the present author.
